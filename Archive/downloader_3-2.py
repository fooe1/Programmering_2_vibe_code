...
def get_chapter_list(series_url: str) -> tuple[str, list[dict]]:
    ...
    with sync_playwright() as pw:
        ...
        # ── Find and focus the scrollable chapter list container ────────────
        # We do this ONCE before scrolling starts so we always have a stable
        # reference to the right element even as its children change.
        # The container is the scrollable ancestor of the chapter links with
        # the largest scrollHeight.
        log.info("  Locating chapter list container …")
        try:
            ...
            () => {{
                ...
                // Walk up ancestors looking for the scrollable container
                // with the most scroll room (scrollHeight - clientHeight)
                let best = null;
                let bestRoom = 0;
                ...
                if (best) {{
                    // Mark it so we can find it again reliably
        ...
        # ── Scroll loop ─────────────────────────────────────────────────────
        # Strategy:
        # 1. Try scrolling the marked container directly via scrollTop
        # 2. If that fails, send PageDown keypress to the focused element
        # 3. Fall back to window scroll
        # We track the container's scrollTop to detect when we've truly
        # reached the bottom (scrollTop stops changing).
        #
        # We stop only when BOTH conditions are true:
        # - scrollTop has not changed for STABLE_ROUNDS consecutive steps
        # - No new chapters were found in that same period
        # This prevents stopping early in a virtualised list where rows
        # are replaced (scrollTop may stay the same temporarily).

        SCROLL_STEP = 150
        PAUSE       = 0.2
        STABLE_ROUNDS = 20   # must see no scroll movement AND no new chapters

        stable = 0
        prev_scroll_top = -1

        while stable < STABLE_ROUNDS:
            # Get current scrollTop of the container
            try:
            ...
            # Attempt to scroll the container
            try:
                page.evaluate(f"""
                () => {{
                    const el = document.querySelector('[data-chapter-scroll-container]');
            ...
            # Also send keyboard PageDown in case the container needs focus
            try:
            ...
            if new > 0:
                ...
                # Scroll moved but no new chapters – keep going, list may be rendering
