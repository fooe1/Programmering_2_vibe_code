...
def _build_chapter_list(raw: list[dict], manga_id: str,
    ...
    # Some manga have multiple scanlation groups per chapter (Alpha, Delta,
    # Gamma etc.) producing separate URLs for the same chapter number.
    # We keep only the first URL seen for each number.
    seen_nums = set()
    deduped = []
    for ch in chapters:
        num = _num(ch)
        if num not in seen_nums:
            ...
            log.info(f"  Skipping duplicate: {ch['title']} ({ch['id']})")

    if len(deduped) < len(chapters):
        log.info(f"  Deduplicated {len(chapters)} -> {len(deduped)} unique chapters.")

    log.info(f"Total: {len(deduped)} chapters for '{manga_title}'")
    return manga_title, deduped
